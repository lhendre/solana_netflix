import { NetflixClone } from '../target/types/netflix_clone'
...
const program = anchor.workspace.NetflixClone as Program<NetflixClone>
...
describe('netflix-clone', () => {
  ...
  it("Create First Video", async () => {
    ...
    try{
      ...
      await program.rpc.addMovie("this is first video", "dummy_url","first", "https://first.com", {
        accounts: {
          state: stateSigner,
          video: videoSigner,
          authority: creatorKey,
  ...
  it("Create Second Video", async () => {
    ...
    try{
      ...
      await program.rpc.addMovie("this is second video", "dummy_url", "second", "https://second.com", {
        accounts: {
          state: stateSigner,
          video: videoSigner,
          authority: creatorKey,
  ...
  it("Videos can be subscribed correctly", async () => {
    const stateInfo = await program.account.stateAccount.fetch(stateSigner);
    console.log(stateInfo.videoCount);
    ...
    try{
      ...
      console.log(videoInfo);
      assert(videoInfo.authority.toString() === creatorKey.toString(), "Video Creator is Invalid");
      expect(videoInfo.subscribe_count).to.equal(0);
      await program.rpc.subscribe(user.publicKey, {
        accounts: {
          video: videoSigner,
        },
        signers: []
      ...
      videoInfo = await program.account.videoAccount.fetch(videoSigner);
      expect(videoInfo.subscribe_count).to.equal(1);
      expect(videoInfo.peopleWhoLiked[0].toString()).to.equal(user.publicKey.toString());

      const secondUser = anchor.web3.Keypair.generate();
      await program.rpc.subscribe(secondUser.publicKey, {
        accounts: {
          video: videoSigner.publicKey,
        },
        signers: []
      ...
      const thirdUser = anchor.web3.Keypair.generate();
      await program.rpc.subscribe(thirdUser.publicKey, {
        accounts: {
          video: videoSigner.publicKey,
        },
        signers: []
      ...
      tweet = await program.account.tweet.fetch(tweetKeypair.publicKey);
      videoInfo = await program.account.videoAccount.fetch(videoSigner);
      expect(videoInfo.subscribe_count).to.equal(3);
  ...
  it('should not allow writting an empty description', async () => {
    ...
    try{
      ...
      await program.rpc.addMovie("", "dummy_url","first", "https://first.com", {
        accounts: {
          state: stateSigner,
          video: videoSigner,
          authority: creatorKey,
